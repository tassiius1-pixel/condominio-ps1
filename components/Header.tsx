...
import {
  LogOutIcon, UsersIcon, BarChartIcon, LayoutDashboardIcon,
  BellIcon, UploadIcon
} from './Icons';
...
const Header: React.FC<HeaderProps> = ({
  ...
  // =============================
  // LOGO PADRÃO E LOGO ATUAL
  // =============================

  // ✔ Logo fixa do Supabase (a que você enviou)
  const defaultSupabaseLogo =
  ...
  // ✔ Fallback padrão caso nenhuma logo exista
  const fallbackLogo =
  ...
  // ✔ Regra:
  // 1) Se já tiver condoLogo → usa
  // 2) Senão → usa logo fixa do Supabase
  // 3) Senão → fallback
  const logoToShow = condoLogo || defaultSupabaseLogo || fallbackLogo;
          ...
          <div className="flex items-center gap-4">
            <div className="relative group flex-shrink-0">
              ...
              {currentUser.role === Role.ADMIN && (
                <div
                  className="absolute inset-0 bg-black bg-opacity-50 rounded-md flex items-center justify-center
          ...
          <div className="flex items-center space-x-2">
            ...
            <nav className="hidden md:flex items-center space-x-2 mr-2">
              <button
                ...
                className={`${navItemClasses} ${currentView === 'dashboard'
              ...
              {[Role.ADMIN, Role.GESTAO].includes(currentUser.role) && (
                <button
                  ...
                  className={`${navItemClasses} ${currentView === 'reports'
                    ? activeNavItemClasses
                    : inactiveNavItemClasses}`}
                >
              ...
              {currentUser.role === Role.ADMIN && (
                <button
                  ...
                  className={`${navItemClasses} ${currentView === 'users'
                    ? activeNavItemClasses
                    : inactiveNavItemClasses}`}
                >
